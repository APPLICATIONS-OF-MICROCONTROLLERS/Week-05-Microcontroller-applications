...
|-------------|-------------------|--------------------|
| Internal SRAM | 0x3ffb5264 | 520,192 |
...
ตอบ SRAM: ใช้เก็บข้อมูลที่เปลี่ยนแปลงได้ระหว่างโปรแกรมทำงาน เช่น ตัวแปรใน stack, heap, global Flash Memory: ใช้เก็บข้อมูลถาวร เช่น โค้ดโปรแกรม และค่าคงที่ (const, string literal)


2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
 ตอบ
 
|ประเภทตัวแปร | ตัวอย่าง | Address | อยู่ในหน่วยความจำ | ช่วงAddress โดยประมาณ |
|-----|-----|-----|-----|-----|
| Stack | stack_var | 0x3FFB4550 | SRAM | 0x3FFBxxxx |
| Global |sram_buffer | 0x3FFB16AC | SRAM | 0x3FFBxxxx |
| Heap | heap_ptr | 0x3FFB5264 | SRAM | 0x3FFBxxxx |
| Flash (const) | flash_string | 0x3F407B64 | Flash | 0x3F40xxxx |

3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
ตอบ SRAM รวมทั้งหมด: 520,192 bytes
ว่าง (Free): ประมาณ 67,812 bytes
ใช้งานไปแล้ว: ประมาณ 452,380 bytes
เปอร์เซ็นต์การใช้งาน SRAM: ≈ 87%
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 8982 μs (sum=3609780224) | 1.00x |
| Random | Internal SRAM | 14120 μs (sum=3609780224) | 0.57x |
| Sequential | External Memory | 48194 μs (sum=3609780224) | 1.00x |
| Random | External Memory | 53746 μs (sum=3609780224) | 0.12x |

**Table 3.2: Stride Access Performance**

| Stride Size | Time (μs) | Ratio vs Stride 1 |
|-------------|-----------|------------------|
| 1 | 9723 μs (sum=3609780224) |  1.00x |
| 2 |  8701 μs (sum=1801306112) | 0.89x |
| 4 | 2967 μs (sum=3044552704) | 0.31x |
| 8 | 1373 μs (sum=3666176000) | 0.14x |
| 16 | 653 μs (sum=1829504000) | 0.07x |

### คำถามวิเคราะห์

1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
ตอบ Sequential access เร็วกว่าเพราะข้อมูลอยู่ติดกัน cache โหลดล่วงหน้าได้ Random access ทำให้ cache miss บ่อย ช้ากว่า

2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
ตอบ Internal SRAM อยู่ในชิป เร็วกว่า External memory อยู่ภายนอก ช้ากว่า ~5 เท่า เหมาะกับข้อมูลใหญ่

3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
ตอบ Stride ใหญ่ขึ้น เวลา access ลดลง เพราะข้ามข้อมูล แต่เข้าถึงข้อมูลน้อยลง  ต้องใช้ให้เหมาะกับงาน
...
| Average Time per Iteration (μs) | 105 μs	 | 9652 μs |
| Total Execution Time (ms) | 4996 ms	 | 5946 ms |
...
| Metric | Value |
|--------|-------|
| Messages Sent | 10 (messages sent by Core 0) |
| Messages Received | 10 (messages received by Core 1) |
| Average Latency (μs) | ~18,000 μs (average between all iterations) |
...
ตอบ Core 0 (PRO_CPU):
เหมาะสำหรับงานที่ต้องการความเร็วในการประมวลผลสูงและมีความละเอียดของเวลา เช่น งานควบคุมฮาร์ดแวร์แบบเรียลไทม์ หรือการตอบสนองที่รวดเร็ว
Core 1 (APP_CPU):
เหมาะสำหรับงานที่สามารถยอมรับความหน่วงได้ เช่น งานประมวลผลทั่วไป หรืองานที่มีความซับซ้อนและสามารถประมวลผลแบบ background ได้

2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
ตอบ ค่า latency เฉลี่ยของการส่งข้อมูลจาก Core 0 ไปยัง Core 1 มีค่าประมาณ 16,000 μs ซึ่งถือว่าค่อนข้างสูง โดยค่าดังกล่าวสะท้อนถึง overhead ที่เกิดจากกลไกการสื่อสารระหว่างคอร์ของ ESP32 ที่ยังมีข้อจำกัดด้านความเร็ว และอาจเป็นปัจจัยที่ลดทอนประสิทธิภาพหากมีการส่งข้อมูลระหว่างคอร์บ่อยครั้ง

3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
ตอบ การกระจายงานยังไม่สมดุล โดย:
Core 0 ทำงานครบ 100 ครั้งใน 4,996 ms แสดงถึงความเร็วและประสิทธิภาพที่ดี
Core 1 ทำงานครบ 150 ครั้งใน 5,946 ms แม้จะทำงานมากกว่า แต่ใช้เวลานานกว่า และมีความหน่วงสูงกว่า
...
**ข้อมูลนักศึกษา:**
- ชื่อ: นาย ธีรวีณ์ ชำนาญดี
- รหัสนักศึกษา: 67030305
- วันที่ทำการทดลอง: 16/08/2025
