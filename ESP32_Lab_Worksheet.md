...
3. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
 ตัวแปรแต่ละตัวอยู่ใน Address Range ตามหน้าที่:
Stack → Local var
Heap → malloc/new
Data segment → Global/static
Flash → string literal/const
4. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
SRAM ใช้ไป 140,096 bytes จากทั้งหมด 520,192 bytes
---
...
เพราะการเข้าถึงแบบ Sequential (ตามลำดับ) สามารถใช้ประโยชน์จาก Cache และ Memory Prefetching ได้อย่างมีประสิทธิภาพสูงสุด
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
Internal SRAM เป็นหน่วยความจำที่อยู่ภายในตัวชิป (เช่น ESP32), ส่วน External Memory เป็นหน่วยความจำภายนอกที่เชื่อมต่อผ่านบัส (เช่น PSRAM)
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
Stride คือ "ระยะห่างของข้อมูลแต่ละตำแหน่ง" ที่โปรแกรมเข้าถึงในหน่วยความจำ
Stride ขนาดเล็ก (เช่น 1, 2)  CPU & Cache ทำงานได้ดี
Stride ใหญ่ (เช่น 8, 16)  CPU ข้าม cache line ไปบ่อย  ทำงานเร็วขึ้นในบางกรณี (เพราะข้อมูลน้อยลงต่อรอบ)
...
Core 0: เหมาะกับงานจริงจัง, time-critical, interrupt, sensor
Core 1: เหมาะกับงานเบื้องหลัง, non-critical, งานประมวลผลหนักที่ไม่เร่งด่วน
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
   Average Latency = 9,523 μs = 9.523 ms ต่อข้อความ
   Queue Overflow Count = 99.7 ครั้ง
3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
ไม่มีประสิทธิภาพ เพราะ Core 1 แบกงานหนักกว่า ถึงแม้จำนวน iterations จะมากกว่าเพียง 50%
แต่เวลาในการประมวลผล ช้ากว่า 143 เท่า
---
