...
1. **Docker Commands**: คำสั่ง `docker-compose up -d` และ `docker-compose exec esp32-dev bash` ทำอะไร?
   `docker-compose up -d`ใช้เพื่อ เริ่มต้น container ตามที่กำหนดไว้ใน docker-compose.yml `docker-compose exec esp32-dev bash`ใช้เพื่อ เข้าไปใน container ที่ชื่อว่า esp32-dev
...
idf.py build → สั่ง compile/build โค้ด

idf.py flash → อัปโหลดโค้ดไปยังบอร์ด ESP32

idf.py monitor → ดู log output จาก ESP32
3. **New Tools**: เครื่องมือใหม่ที่ติดตั้ง (tree, htop) ใช้ทำอะไร?
แสดง โครงสร้างโฟลเดอร์/ไฟล์ แบบเป็นรูปต้นไม้
4. **Architecture Focus**: การศึกษา ESP32 architecture แตกต่างจากการทำ arithmetic ใน Lab4 อย่างไร?
lab4 จะเป็นการคำนวณเบื้องต้น ให้ฝึกเขียนโปรแกรมและใช้งานESPเบื้องต้น
...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 5284| 1.00x |
| Random | Internal SRAM | 5073 | 0.96x |
| Sequential | External Memory | 4919 | 0.96x |
| Random | External Memory | 4723| 0.96x |
...
1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
    ข้อมูลทั้งหมดอยู่ใน cache line เดียวกัน หรือใกล้กัน → โหลดมาครั้งเดียว ใช้ได้หลายครั้ง → เร็ว
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
    Internal SRAM เร็วและใกล้ CPU → เหมาะกับ data ที่ใช้บ่อย
   External memory ช้ากว่าแต่เก็บข้อมูลได้เยอะกว่า
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
Stride คือระยะห่างระหว่างแต่ละ access (เช่น A[0], A[4], A[8] → stride = 4)
...
| Average Time per Iteration (μs) | 104 | 9695 |
| Total Execution Time (ms) | 4994 | 5946 |
| Task Completion Rate | 20.0 | 25.2 |
...
Core 0 (PRO_CPU)	งาน protocol logic, checksum, ส่ง message ทุก 10 รอบ	104 µs
Core 1 (APP_CPU)	งาน floating-point computation (sqrt), รับ message	9695 µs
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
Inter-core communication บน ESP32 แบบ polling ผ่าน Queue มี overhead ~15 ms ต่อข้อความ ซึ่งถือว่าต่ำพอสมควรในระบบ real-time แบบไม่เร่งด่วน
...
**ข้อมูลนักศึกษา:**
- ชื่อ: นางสาวสราสินี สิทธิสาร
- รหัสนักศึกษา: 67030236
- วันที่ทำการทดลอง: 1/8/2025
...
1. เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:
   ในสัปดาห์ที่4การใช้dockerยังไมม่มีความชำนาญมากนัก

2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture:
   ทำให้รู้ว่าcoreแต่ละตัวมีหน้าที่เฉพาะในการทำงาน

3. ความท้าทายที่พบในการทำ architecture analysis:
   ต้องมีการวิเคราะห์และตรวจสอบให้ละเอียอดก่อนรันโค้ด เพราะถ้าหากเขียนผิดแค่นิดเดียวจะทำให้ไม่สามารถรันได้ และต้องเข้าใจโครรงสร้างของmemory เพื่อbalance load อย่างมีประสิทธิภาพ
